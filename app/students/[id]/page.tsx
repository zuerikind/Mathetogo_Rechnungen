...
export default function StudentDetailPage() {
  ...
  const subscriptionBillingInputs = useMemo(
    () =>
      subscriptions.map((s) => ({
        id: s.id,
  ...
  const monthBreakdown = useMemo(() => {
    return monthOptions.map((m) => {
      ...
      const sub = subscriptionProrationForMonth(subscriptionBillingInputs, year, m.value);
      return { month: m, sessions: rows.length, hours: totalMinutes / 60, chf: totalCHF + sub };
    });
  }, [sessionsForYear, year, subscriptionBillingInputs]);

  const totalAllTime =
    allSessions.reduce((acc, s) => acc + s.amountCHF, 0) +
      ...
      {loading ? (
        ...
        <div className="min-w-0 space-y-4">
          <header className="rounded-lg border border-gray-200 bg-white p-4">
            <h2 className="break-words text-xl font-semibold text-gray-900">{student.name}</h2>
