...
export function InvoiceHistoryClient() {
  ...
  const [pendingRows, setPendingRows] = useState<PendingDeletionRow[]>([]);
  const [pendingTotalCHF, setPendingTotalCHF] = useState(0);
  ...
  const loadPending = useCallback(async () => {
    try {
      ...
      setPendingRows(Array.isArray(body.rows) ? body.rows : []);
      setPendingTotalCHF(typeof body.totalCHF === "number" ? body.totalCHF : 0);
    } catch {
      // Das Band ist eine Warnung, kein Kerninhalt — ein Fehlschlag darf die Liste nicht stören.
    }
  }, []);

  /** Entscheid über Vormerkungen; danach Liste und Beträge neu laden. */
  const resolvePending = async (action: "confirm" | "reject", sessionIds: string[]) => {
    ...
    try {
      ...
      if (res.redirected || !res.ok) {
        alert(res.redirected ? "Sitzung abgelaufen — bitte neu anmelden." : (body.error ?? "Aktion fehlgeschlagen."));
        return;
      }
          ...
          <section className="rounded-2xl border-2 border-red-300 bg-red-50 p-4 shadow-sm sm:p-5">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <h2 className="text-base font-bold text-red-900">
                ⚠ {pendingRows.length} Lektion{pendingRows.length === 1 ? "" : "en"} sollen entfernt werden
              </h2>
              <span className="text-sm font-semibold text-red-900">
                {formatAmount(pendingTotalCHF)}
              </span>
            </div>
                ...
                >
                  ...
                  <span className="flex shrink-0 gap-2">
                    <button
                      ...
                      onClick={() => void resolvePending("confirm", [row.id])}
                    ...
                    <button
                      type="button"
                      disabled={pendingBusy}
                      onClick={() => void resolvePending("reject", [row.id])}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                type="button"
                disabled={pendingBusy}
                onClick={() => void resolvePending("confirm", pendingRows.map((r) => r.id))}
              ...
              >
                {pendingBusy ? "Läuft…" : `Alle ${pendingRows.length} entfernen`}
              </button>
              <button
                type="button"
                disabled={pendingBusy}
                onClick={() => void resolvePending("reject", pendingRows.map((r) => r.id))}
